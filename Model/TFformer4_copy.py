...
class TFformer4(nn.Module):
    ...
    def __init__(self, T, depth, heads, chs_num, class_num, tt_dropout, ff_dropout, dim_thead=8, dim_fhead=8, dim=220):
        ...
        for _ in range(1):
            # self.crossAttentionEncoder1.append(ModuleList([
            #     CrossAttention(chs_num*2, self.F[1], 8, 8, 8),
            #     nn.LayerNorm(chs_num*2),
            #     FeedForward(chs_num*2, hidden_dim=16, dropout=ff_dropout),
            #     nn.LayerNorm(chs_num*2)
            # ]))
            self.crossAttentionEncoder1.append(crossAttentionEncoder(chs_num*2, self.F[1], 8, 8, 8, 16))

        self.crossAttentionEncoder2 = []
        for _ in range(1):
            # self.crossAttentionEncoder2.append(ModuleList([
            #     CrossAttention(self.F[1], chs_num * 2, 8, 8, 8),
            #     nn.LayerNorm(self.F[1]),
            #     FeedForward(self.F[1], hidden_dim=16, dropout=ff_dropout),
            #     nn.LayerNorm(self.F[1])
            # ]))
            self.crossAttentionEncoder2.append(crossAttentionEncoder(self.F[1], chs_num * 2, 8, 8, 8, 16))
        ...
        self.mlp_head2 = nn.Sequential(
            nn.Flatten(),
            nn.Dropout(self.dropout_level),
            nn.Linear(output_dim * self.F[1], class_num * 2),
