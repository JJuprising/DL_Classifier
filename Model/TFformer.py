...
#     def __init__(self, input_dim_kv, input_dim_q, hidden_dim):
#         super(CrossAttention, self).__init__()
#         self.linear_kv = nn.Linear(input_dim_kv, hidden_dim)
#         self.linear_q = nn.Linear(input_dim_q, hidden_dim)
#
#     def forward(self, x_t, x_fft):
#         # 计算键值对序列 K、V，使用时序特征 x_t
#         kv = self.linear_kv(x_t)  # (batch_size, seq_len_t, hidden_dim)
#
#         # 计算查询序列 Q，使用频域特征 x_fft
#         q = self.linear_q(x_fft)  # (batch_size, seq_len_fft, hidden_dim)
#
#         # 计算注意力权重
#         scores = torch.matmul(kv, q.transpose(1, 2))  # (batch_size, seq_len_t, seq_len_fft)  # (batch_size, seq_len_fft, seq_len_t)
#         attentions = F.softmax(scores, dim=-1)  # 在维度2上进行softmax，归一化为注意力权重 (batch_size, seq_len_fft, seq_len_t)
#
#         attention_expanded = attentions.expand(-1, -1, x_t.size(-1))  # 广播操作，形状变为 (30, 1, 12)
#         output = attention_expanded + x_t# (batch_size, seq_len_fft, hidden_dim)
#
#         return output
#
#
...
#         input_dim_kv = x_t.shape[-1]  # 时序特征的维度
#         input_dim_q = x_fft.shape[-1]  # 频域特征的维度
#         hidden_dim = 12
#         cross_attention = CrossAttention(input_dim_kv, input_dim_q, hidden_dim)
#
#         # 前向传播
#         output = cross_attention(x_t, x_fft).squeeze(1)
#
#         return   output
...
class FeatureFusionGRU(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super(FeatureFusionGRU, self).__init__()
        self.gru_t = nn.GRU(input_dim, hidden_dim, batch_first=True)
        self.gru_fft = nn.GRU(input_dim, hidden_dim, batch_first=True)
        self.fc = nn.Linear(hidden_dim * 2, output_dim)

    def forward(self, x_t, x_fft):
        # 时域特征的GRU
        _, h_t = self.gru_t(x_t)

        # 频域特征的GRU
        _, h_fft = self.gru_fft(x_fft)

        # 将GRU的隐藏状态拼接在一起
        h = torch.cat((h_t, h_fft), dim=-1)

        # 通过全连接层进行融合
        fused_output = self.fc(h)
...
class TFformer(Module):
    ...
    def __init__(self, T, depth, heads, chs_num, class_num, tt_dropout, ff_dropout, dim_thead=8, dim_fhead=8, dim=220):
        ...
        # 结果融合层
        # self.fusion_layer = nn.Conv1d(2, 1, kernel_size=1)
        self.fusion_layer = FeatureFusionGRU(12, 16, 12)
    ...
    def forward(self, x):
        ...
        # # 加权求和
        # fused_output = weighted_x_t + weighted_x_fft
        output = self.fusion_layer(x_t,x_fft)
